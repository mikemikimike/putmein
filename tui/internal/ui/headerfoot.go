...
func RenderHeader(width int, sessionName string, msgCount int) string {
	brand := HeaderBrandStyle.Render("⬡ PUTDEV") +
...
func renderInputLine(query string, cursorPos int, loading bool) string {
	runes := []rune(query)
	n := len(runes)
	...
	if cursorPos < n {
		...
		// Cursor is at end — draw a block
		if n == 0 {
			// Empty field: block then placeholder
			sb.WriteString(FooterCursorStyle.Render("█"))
			sb.WriteString(FooterHintStyle.Copy().PaddingLeft(0).Render(" Ask Ozias something…"))
		} else {
			sb.WriteString(FooterCursorStyle.Render("█"))
		}
	}
...
func RenderFooter(width int, query string, cursorPos int, loading bool) string {
	innerW := width - 6
	if innerW < 4 {
		innerW = 4
	}

	inputLine := renderInputLine(query, cursorPos, loading)

	var promptStr string
	if loading {
		promptStr = FooterPromptLoadingStyle.Render("❯ ")
		line := promptStr + FooterHintStyle.Copy().PaddingLeft(0).Render("Ozias is working…")
		box := FooterBoxLoadingStyle.Width(innerW).Render(line)
		hint := FooterHintStyle.Render("/help · /sessions · /new · /exit")
		return "\n" + box + "\n" + hint
	}

	promptStr = FooterPromptStyle.Render("❯ ")
	line := promptStr + inputLine
	box := FooterBoxStyle.Width(innerW).Render(line)
