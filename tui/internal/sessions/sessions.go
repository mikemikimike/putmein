...
// SessionsDir is where all session JSON files live — same path as the React TUI.
var SessionsDir = filepath.Join(os.Getenv("HOME"), ".putdev", "sessions")

// SessionMessage mirrors the React TUI's SessionMessage interface exactly.
type SessionMessage struct {
	Role      string `json:"role"`      // "user" | "assistant" | "terminal"
	Content   string `json:"content"`
	Thinking  string `json:"thinking,omitempty"`
	Timestamp string `json:"timestamp"`
}

// Session mirrors the React TUI's Session interface exactly.
type Session struct {
	ID        string           `json:"id"`
...
// Create builds a new empty session with a timestamp-based name.
...
// Update returns a copy of the session with updated messages + UpdatedAt.
func Update(s *Session, messages []SessionMessage) *Session {
	updated := *s
...
// Summary returns a short summary string of the first user message.
