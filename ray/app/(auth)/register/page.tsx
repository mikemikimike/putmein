  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="font-jersey text-4xl text-white mb-2 tracking-wide">Create account</h1>
        <p className="text-sm" style={{ color: "rgba(255,255,255,0.3)" }}>
          Get started with PutmeIn ray in seconds
        </p>
      </div>
      ...
      >
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <label className="ray-eyebrow">Full Name</label>
            <input className="ray-input" type="text" placeholder="John Doe" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required autoFocus autoComplete="name" />
          </div>

          <div className="flex flex-col gap-2">
            <label className="ray-eyebrow">Email</label>
            <input className="ray-input" type="email" placeholder="you@example.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required autoComplete="email" />
          </div>

          <div className="flex flex-col gap-2">
            <label className="ray-eyebrow">Password</label>
            <input className="ray-input" type="password" placeholder="Min. 8 characters" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required autoComplete="new-password" />
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              style={passwordsMatch ? { borderColor: "rgba(239,68,68,0.35)" } : {}}
            />
            {passwordsMatch && <p className="text-xs" style={{ color: "#fca5a5" }}>Passwords don&apos;t match</p>}
          </div>
        ...
        <div className="text-center text-sm" style={{ color: "rgba(255,255,255,0.3)" }}>
          ...
          <Link href="/login" className="font-medium text-white transition-opacity hover:opacity-70">Sign in</Link>
        </div>
      </div>
